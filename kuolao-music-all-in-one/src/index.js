...
const LX_PLATFORM_MAP = { kw:'kuwo', kuwo:'kuwo', tx:'qq', qq:'qq', kg:'kugou', kugou:'kugou', wy:'netease', netease:'netease', '163':'netease' };
function lxPlatform(value){ return LX_PLATFORM_MAP[String(value||'').toLowerCase()]||''; }
function decodeLiteral(raw){
  try {
    const quote=raw[0];
    if(quote==='"') return JSON.parse(raw);
    if(quote==="'") return JSON.parse('"'+raw.slice(1,-1).replace(/\\/g,'\\\\').replace(/"/g,'\\"').replace(/\\\\'/g,"'")+'"');
    if(quote==='`') return raw.slice(1,-1).replace(/\\`/g,'`').replace(/\\n/g,'\n').replace(/\\x([0-9a-fA-F]{2})/g,(_,h)=>String.fromCharCode(parseInt(h,16))).replace(/\\u([0-9a-fA-F]{4})/g,(_,h)=>String.fromCharCode(parseInt(h,16)));
  } catch {}
  return raw.slice(1,-1);
}
function normalizeLxTemplate(url){
  let out=String(url||'').trim();
  out=out.replace(/\$\{(?:musicItem\.)?(?:songmid|hash|rid|id|audio_id|copyrightId)[^}]*\}/gi,'{id}')
    .replace(/\$\{(?:quality|br|level)[^}]*\}/gi,'{level}')
    .replace(/\$\{(?:type|format)[^}]*\}/gi,'{type}');
  if(!out.includes('{id}')){
    out=out.replace(/([?&](?:id|rid|mid|songmid|hash|audio_id|copyrightId)=)[^&"'`\s+]*/i,'$1{id}');
  }
  if(!out.includes('{level}') && /[?&](?:level|quality|br)=/i.test(out)) out=out.replace(/([?&](?:level|quality|br)=)[^&"'`\s+]*/i,'$1{level}');
  if(!out.includes('{type}') && /[?&](?:type|format)=/i.test(out)) out=out.replace(/([?&](?:type|format)=)[^&"'`\s+]*/i,'$1{type}');
  return out;
}
function parseLxJsonConfig(payload){
  const root=payload?.sources||payload?.data?.sources||payload;
  if(!root||typeof root!=='object'||Array.isArray(root)) return [];
  const out=[];
  for(const [key,value] of Object.entries(root)){
    const platform=lxPlatform(key)||lxPlatform(value?.platform||value?.source);
    if(!platform||!value) continue;
    const play=normalizeLxTemplate(value.url_template||value.play_url_template||value.playUrl||value.musicUrl||value.url||'');
    const search=String(value.search_url_template||value.searchUrl||'').trim();
    if(validSourceTemplate(play)) out.push({platform,name:String(value.name||`洛雪-${key}`).slice(0,80),url_template:play,search_url_template:validSearchTemplate(search)?search:'',adapter_config:{version:1,lx:{mode:'json',source:key}}});
  }
  return out;
}
function detectLxPlatformNear(code,index){
  const near=code.slice(Math.max(0,index-350),Math.min(code.length,index+150)).toLowerCase();
  const checks=[['kuwo',['\'kw\'','"kw"','kw:','kuwo']],['qq',['\'tx\'','"tx"','tx:','qqmusic','qq音乐']],['kugou',['\'kg\'','"kg"','kg:','kugou','酷狗']],['netease',['\'wy\'','"wy"','wy:','netease','网易云']]];
  for(const [platform,needles] of checks) if(needles.some(x=>near.includes(x))) return platform;
  return '';
}
function parseLxJavascript(code){
  ...
  while((m=literalRe.exec(code))){
    ...
    const template=normalizeLxTemplate(decoded);
    if(!validSourceTemplate(template)) continue;
    const platform=detectLxPlatformNear(code,m.index);
    if(!platform) continue;
    const key=platform+'|'+template; if(seen.has(key)) continue; seen.add(key);
    out.push({platform,name:`洛雪-${platform}`,url_template:template,search_url_template:'',adapter_config:{version:1,lx:{mode:'static',sourceUrl:''}}});
  }
  return out;
}
async function readTextLimited(response,maxBytes=512*1024){
  const len=Number(response.headers.get('content-length')||0); if(len>maxBytes) throw new Error('洛雪音源脚本过大');
  const text=await response.text(); if(text.length>maxBytes) throw new Error('洛雪音源脚本过大'); return text;
}
function parseKnownLxRuntimeScript(code){
  const out=[];
  // Huibq/keep-alive render_api.js style:
  // API_URL + /url/{source}/{songId}/{quality}, authenticated by X-Request-Key.
  const apiMatch=code.match(/const\s+API_URL\s*=\s*(['\"`])([^'\"`]+)\1/);
  const keyMatch=code.match(/const\s+API_KEY\s*=\s*(['\"`])([^'\"`]+)\1/);
  if(!apiMatch) return out;
  const base=String(apiMatch[2]||'').replace(/\/+$/,'');
  if(!/^https:\/\//i.test(base)) return out;
  const apiKey=String(keyMatch?.[2]||'');
  const sourceCodes={kuwo:'kw',qq:'tx',kugou:'kg',netease:'wy'};
  const scriptLooksCompatible=/\/url\/\$\{source\}\/\$\{songId\}\/\$\{quality\}/.test(code)
    || /\/url\/[^/]+\/[^/]+\/[^/]+/.test(code)
    || (code.includes('handleGetMusicUrl') && code.includes('X-Request-Key'));
  if(!scriptLooksCompatible) return out;
  for(const [platform,source] of Object.entries(sourceCodes)){
    out.push({
      platform,
      name:`洛雪-Huibq-${source}`,
      url_template:`${base}/url/${source}/{id}/{quality}`,
      search_url_template:'',
      adapter_config:{version:1,lx:{mode:'huibq-render-api',source,sourceUrl:''},play:{mode:'json',resultPath:'url',headers:apiKey?{'X-Request-Key':apiKey,'Content-Type':'application/json'}:{'Content-Type':'application/json'}}}
    });
  }
  return out;
}
...
async function importLxSource(input){
  let text=String(input?.script||'').trim(), sourceUrl=String(input?.source_url||'').trim();
  if(sourceUrl){ assertSafeRemoteUrl(sourceUrl); const res=await fetchLimited(sourceUrl,{headers:{accept:'application/javascript,text/javascript,application/json,text/plain'}},10000); if(!res.ok) throw new Error(`下载洛雪音源失败：HTTP ${res.status}`); text=await readTextLimited(res); }
  if(!text) throw new Error('请填写洛雪音源地址或脚本内容');
  let parsed=[];
  try { parsed=parseLxJsonConfig(JSON.parse(text)); } catch {}
  if(!parsed.length) parsed=parseKnownLxRuntimeScript(text);
  if(!parsed.length) parsed=parseLxJavascript(text);
  const obfuscated=/_0x[0-9a-f]{4,}/i.test(text)||text.includes('globalThis[\'lx\']')||text.includes('globalThis.lx');
  for(const item of parsed) item.adapter_config.lx.sourceUrl=sourceUrl;
  return {items:parsed,obfuscated,hasLxRuntime:/globalThis(?:\[['"]lx['"]\]|\.lx)/.test(text)};
}
...
async function api(request, env, url) {
  ...
  if (url.pathname === '/api/sources/lx/import' && method === 'POST') {
    const b=await bodyJson(request); const imported=await importLxSource(b||{});
    if(!imported.items.length){
      const reason=imported.obfuscated?'该洛雪脚本经过混淆且不属于已支持的安全适配格式；请使用 Huibq render_api.js、未混淆模板脚本或洛雪兼容 JSON。':'没有从脚本中识别到可用的 HTTPS 播放适配器';
      return json({ok:false,error:reason,obfuscated:imported.obfuscated,has_lx_runtime:imported.hasLxRuntime},422);
    }
    const count=await env.DB.prepare('SELECT COUNT(*) AS c FROM music_sources WHERE user_id=?').bind(user.id).first();
    const remaining=20-(+count.c); if(remaining<=0) return error('每个用户最多 20 个音源');
    const saveList=imported.items.slice(0,remaining), now=Math.floor(Date.now()/1000), saved=[];
    for(const item of saveList){
      ...
      const result=await env.DB.prepare('INSERT INTO music_sources (user_id,name,url_template,is_selected,created_at,platform,search_url_template,source_kind,adapter_config) VALUES (?,?,?,?,?,?,?,?,?)')
        .bind(user.id,item.name,item.url_template,existingSelected?0:1,now,item.platform,item.search_url_template,'lx',JSON.stringify(item.adapter_config)).run();
      saved.push({id:result.meta.last_row_id,platform:item.platform,name:item.name,url_template:item.url_template});
    }
    return json({ok:true,saved,obfuscated:imported.obfuscated,has_lx_runtime:imported.hasLxRuntime},201);
  }
  ...
  if (url.pathname === '/api/resolve' && method === 'GET') {
    ...
    const config=parseAdapterConfig(src.adapter_config);
    const quality=level==='standard'?'128k':'320k';
    const target=sourceTemplate(src.url_template,{id:songId,level,quality,br:level==='lossless'?'999':level==='exhigh'?'320':'128',type}); assertSafeRemoteUrl(target);
    const mode=config.play?.mode||'direct';
    if(mode==='direct'||mode==='redirect') return Response.redirect(target,302);
    const configuredHeaders=(config.play?.headers&&typeof config.play.headers==='object')?config.play.headers:{};
    const res=await fetchLimited(target,{headers:{accept:'application/json',...configuredHeaders}}); if(!res.ok)return error(`音源接口返回 HTTP ${res.status}`,502);
